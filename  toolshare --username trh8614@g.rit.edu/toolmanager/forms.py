...
class ToolForm( forms.Form ):
    ...
    quantity = forms.IntegerField(required=True,validators=[MinValueValidator(int('1'))], error_messages={'invalid': _("A quantity must be entered or the quantity is not above zero.")})

    quantity_available = forms.IntegerField(error_messages={'invalid': _("A quantity must be entered.")},
                                   label = _("Quantity Available"),
                                   required=True,validators=[MinValueValidator(int('1'))])
